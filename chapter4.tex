...
\end{equation}
This is defined by Scheeres and Sanchez, and applies Scheeres notation for YORP spin acceleration \cite{Scheeres2014}\cite{Scheeres2007}. Mentioning these barriers to YORP dynamics is important because the original intention of defining the YORP effect was to explain how small meteroids are accelerated to the point of disruption \cite{Rubincam2000}. We extend it to small rubble-pile asteroids to explain the unmodeled accelerations observed in their dynamics.
...
\begin{table}[H]
    ...
    \resizebox{\textwidth}{!}{
        ...
        \hline
        \makegapedcells
        Bennu & 1260 $kg/m^3 $ \cite{Chesley2014} & 1 Pa \cite{Walsh2022} & 282.37 m & $35^{\circ}$ \cite{Zhang2022} \\ 
        \hline 
        \makegapedcells 
        Itokawa & 2000 $kg/m^3$ \cite{Sanchez2014} & 25 Pa \cite{Sanchez2014}  & 267.5 m & 40 \cite{Aoki2014}\\  
        \hline   
        \end{tabular}
        }
...
Comparing the differences between neighboring boulders gives a more random relationship. It is shown that a boulder at a higher latitude could have a lower or higher amount of torque changing the body's obliquity. The largest change in $\dot{i}_s$ is $7.06\times 10^{-6} rad/day^2$. If this change was applied to the largest obliquity changing boulder, in this case, a boulder exhibiting $1.05\times 10^{-5} rad/day^2$, then the variation in obliquity angle shifting due to just this single boulder could induce angular rates faster than the current spin rate within 4.9 million years. This assumes uniform spin is independent of obliquity angle change. These dynamics are naturally coupled and calculations here serve to highlight the magnitude of change due to moving boulders on the surface. In future work, it will be highlighted how necessary it will be to simulate boulders moving positions as the dynamics change due to YORP.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{fig/obliq_delta_boulder_motion_bennu.png}
    \caption{The change in YORP obliquity acceleration from boulder motion with the latitude degree change shown on the x-axis}
