...
\section{Comments on Orbit Averaging}
...
Here we show the change in YORP spin for boulders with the same reported diameter but increasingly sloped tops. This changes the efficiency of radiation by these shapes without changing the diameter that can be ascribed to the boulder. This is just one example of a dimension that can change and reduce the radiative efficiency of our flat faces. Some future examples could be a fully trapezoidal shape, a rounded cap top boulder, or a buried sphere.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{fig/rounding_top_boulder.png}
    \caption{Area reduced by shaving off triangle areas from boulder tops and the subsequent reduction in the YORP spin coefficient}
    \label{fig:efficiency_plot}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{fig/rounding_boulder_tops.png}
    \caption{Graphical representation of area removal causing reduced YORP torque for more realistic boulder shapes where the percentage is the amount of face area leftover after removal versus the original flat wedge face}
    \label{fig:efficiency_drawing}
\end{figure}
We see in Fig.\ref{fig:efficiency_plot}, as we see weathering increase and the top corners being removed, there is a proportional reduction in the YORP spin torque from this boulder. These results are all for boulders with the same reported diameter. This is meant to show a scaling factor for boulders to translate the results given here into their realistic counterparts. For this wedge shape, removing the top corners results in a 20\% reduction in YORP torque, and that's a first order estimate for what to expect when translating our highly asymmetric boulder results to real shapes. We show the geometric procedure taken to remove corners in Fig.\ref{fig:efficiency_drawing} and report the percent area taken away with each step, removing from left to right. Studying multiple types of shapes is an additional effort left to future work, but here we can say that dependent on weathering, we can expect any given boulder to be reduced in YORP spin torque efficiency between 0-20\%. This was analyzed for a boulder on the equator where YORP spin influence is maximized for oblate shapes. These bounds stay proportional for boulders at higher latitudes but the numerical YORP torque reduced per boulder is less due to the lever arm distance being shorter towards the poles.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{fig/spins_boulders_moving.png}
